<?php 
    ...
    class Shop extends CI_Controller{
        ...
        public function editShop(){
            ...
            $resshop = $query->result_array();
            $query = $this->db->get_where('zs_classtype',array('pid !='=>0));
            $restype = $query->result_array();

            $this->load->view('admin/head.php' ,array('site'=>'编辑课程信息'));
            $this->load->view('admin/Shop/editShop.php',array('Shop'=>$resshop[0],'type'=>$restype));
            $this->load->view('admin/foot.php');
        }
        ...
        public  function doeditShop(){
            ...
            if($this->form_validation->run() == true){
                ...
                $this->db->where('id',$id);
                $res = $this->db->update('zs_classinfo',$data);
                if($res){
                    $this->load->view("admin/notice.php",array('url'=>"/admin/Shop/Shoplist?typeid={$data['typeid']}",'msg'=>'商品修改成功','status'=>'1'));
                }else{
                    $this->load->view("admin/notice.php",array('url'=>"/admin/Shop/editShop?id={$id}",'msg'=>"商品修改失败\r\n联系管理员",'status'=>'0'));
                }
            }else{
                $msg=validation_errors();
